##------------------------------------------------------------------------------
## Setting paths (UPDATED for Modular Engine)
##------------------------------------------------------------------------------

# Use normalizePath to get the absolute path of the directory
workspace_dir <- normalizePath(getwd())

## Link to the system
engine_root <- normalizePath("../../../SWAT-Workflow-R/_Workflow/1_Setup")
...
res_path <- file.path(workspace_dir, "Temp_")
## Data folder
data_path <- file.path(workspace_dir, "Data")
## Scripts folder
lib_path <- file.path(engine_root,  "Libraries")
...
# weather_path <- paste0(data_path, '/for_prepr/Weather_data_Lupawa_prepR1.xlsx')
...
dem_path <- paste0(out_path, buildr_data, 'DTM5m_v2.tif')

##Soil raster layer and soil tables paths
soil_layer_path  <- paste0(out_path, buildr_data, 'soil_v2.tif')
